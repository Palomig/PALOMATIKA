...
- `topic_15.json`–`topic_18.json` (геометрия): поле `svg` не редактировать руками — регенерируется через `php artisan svg:bake {id}`. Остальные поля (`geometry`, `params`, текст, ответ) редактируются напрямую в этом же файле
