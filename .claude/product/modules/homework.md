...
Экран `/homework/assignment/{assignment}` — ответы по попыткам, страницы решения плитками (превью `?w=400`, клик — просмотрщик поверх страницы), заметки. Условия задач содержат LaTeX в `$…$`: страница подключает KaTeX и рендерит точечно по классу `math-scope` — по всему `body` нельзя, в заметках учителя доллар это доллар.

Просмотрщик тетради живёт в партиале `pwa/_shared/photo-viewer.blade.php` и переиспользуется на экранах урока. Партиал своего состояния не заводит — окружающий `x-data` обязан дать `photos`, `viewer`, `vi`, `close()`, `step(d)`; `open()` у каждого экрана свой, источники фото разные.
...
## Вторая стадия: разбор на уроке (с 2026-08-13)

Проверка домашки не заканчивается отметкой «проверено». Из всех заданий учитель выбирает те, что надо разобрать с учеником, и они становятся повесткой ближайшего урока.

**Сущность** — `homework_review_items` (модель `HomeworkReviewItem`), отдельная таблица:

| Статус | Что значит |
|---|---|
| `pending` | отмечено на странице проверки, ещё никуда не уехало |
| `planned` | учитель добавил пункт в конкретный урок (`lesson_session_id`) |
| `done` | этот урок завершён — `LessonSessionService::end()` проставляет `resolved_at` |

Почему не флаг на `homework_topic_task_submissions`: **несданную задачу отметить было бы нельзя** — записи о попытке нет, а именно её («не понял, не приступил») разбирать нужнее всего.

Почему не `student_notes` с видом `todo`: у заметки и отметки разный срок жизни. «Не видит подобия» живёт в карточке ученика годами, «разобрать задачу 2» гаснет после ближайшего урока. Связь при этом есть — галочка «записать и в карточку ученика» создаёт рядом обычную `student_note`, её id ложится в `student_note_id`.

**Уникального индекса на пару (назначение, задача) намеренно нет:** разобранную на прошлом уроке задачу можно отметить снова. Активная строка ищется скоупом `->active()` (`pending` + `planned`).

**Четыре экрана:**

1. `/homework/assignment/{id}` — переключатель «Разобрать» на карточке задачи (одно нажатие, текст необязателен), счётчик «К разбору: N» в шапке. Задачи с ошибкой или несданные подсвечиваются жёлтой полосой слева как кандидаты, но **не отмечаются автоматически**.
2. `/lessons/{id}` (draft) — блок «Разбор домашки» над списком задач: очередь `pending` по участникам, чекбоксы, «Добавить в урок».
3. `/lessons/{id}` (live) — те же карточки раскрыты: условие, эталон, ответ ученика, тетрадь, заметка учителя, ссылка на всю домашку.
4. Экран урока ученика — блок «Разбор с учителем»: **только чтение**, поля ответа нет, заметка учителя не отдаётся. Видит только свои карточки — как персональные задачи урока.

**`lesson_session_tasks` не трогается вообще.** Там у каждой строки есть `bank`+`refs` и поле ответа, а разбор — это «смотрим на то, что уже написано». Побочный плюс: живой грид урока, «следующая задача», подсчёт решённых и `LessonHomeworkSuggestionService` остались нетронутыми.

**Сборка карточки — `HomeworkReviewService`**, один источник для всех трёх экранов. Маршрут к фото выбирается по явному параметру `VIEWER_TEACHER` / `VIEWER_STUDENT`, а не по `auth()`: карточки собираются и для чужого экрана.

### Доступ ученика к фото — единственное опасное место

Маршрут `pwa.student.homework.solution-photo` (`StudentController::homeworkSolutionPhoto`) проверяет цепочку фото → попытка → назначение → `student_id`.

**Проверять «то же ДЗ» нельзя:** домашка выдаётся на класс — один `Homework`, много `HomeworkAssignment`, — и такая проверка открыла бы тетради всех одноклассников. Случай закреплён тестом `test_classmate_with_the_same_homework_is_still_rejected`.

Сами фото секретом не считаются (решение Стаса от 2026-08-13) — ограничение ровно одно: ученик видит только свою домашку.
...
- `tests/Feature/HomeworkReviewItemsTest.php` — модель пунктов разбора: статус по умолчанию, повторная отметка после `done`, скоуп `active()`
- `tests/Feature/HomeworkReviewToggleTest.php` — переключатель «Разобрать»: отметка и снятие, зеркалирование заметки в карточку ученика, чужой учитель, задача из другого ДЗ, рендер счётчика и следа «разобрано»
- `tests/Feature/HomeworkReviewServiceTest.php` — сборка карточки и цикл `pending → planned → done`, включая гашение при завершении урока
- `tests/Feature/LessonReviewPanelTest.php` — эндпоинты урока: предложения только по участникам, идемпотентность повторного добавления, возврат в очередь, чужой урок
- `tests/Feature/StudentLessonReviewTest.php` — разбор в `/lessons/{id}/state`: только свои карточки, только `planned`, заметка учителя не утекает
- `tests/Feature/StudentSolutionPhotoAccessTest.php` — **доступ к фото**: своё видно, чужое 403, одноклассник по тому же `Homework` тоже 403, гость на логин
...
## Домашка по скиллам (с 2026-09-20)

Шит «📚 Домашка» на экране урока (`lesson-prep.blade.php`) получил переключатель
**По уроку / По скиллам**. «По уроку» — прежние аналоги разобранных задач; «По скиллам» —
навык из банка «Скиллы» → четыре группы примеров (свёрнуты до шести карточек, «Ещё N»),
кнопки «По 3 из каждой» и «Случайные 10». Кнопка больше не требует задач на уроке: без
них шит открывается сразу на скиллах. Ученики, срок и отправка общие; отмеченное из обоих
режимов уходит одной домашкой (`picker_tasks`, bank=`skills`), заголовок
«ДЗ по скиллам DD.MM — <навык>». Данные пикер берёт из `/lessons/picker-options?bank=skills`.
