...
**Состояние: 7786 заданий в банке ФИПИ → 7762 в выгрузке (24 без рисунка,
который ФИПИ не отдал) → 6816 задач в 538 заданиях 21 темы в базе данных.
946 задач классификатор не отнёс ни к одному номеру и они не импортируются.
Ответов НЕТ НИ У ОДНОЙ задачи**, поэтому весь банк лежит в `draft`: ученику
он не показывается, в picker'е учителя вкладка «10–11 ЕГЭ (Б)» появится
только тогда, когда в теме будет хотя бы одна задача с ответом (фильтр
`supportedTasks` требует ответ). Это сознательный порядок: каркас переносим
сразу, ответы добираем следом, импорт идемпотентен и подхватит их при
повторном прогоне.
