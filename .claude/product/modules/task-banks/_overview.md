...
**Чертежи остаются картинками.** Это расходится с принципом «SVG — основа
проекта»: в ОГЭ каждый растр заменён своим SVG, но их рисовали месяцами, а
для ЕГЭ таких SVG нет. Выкинуть рисунок нельзя — без него условие
нерешаемо, поэтому файлы раскладываются в `public/ege-bank/img`. Замена на
SVG — отдельный этап, модель для неё предусмотрена (`tasks.svg_model`).
