...
<div class="test-page"
  ...
  <div class="test-topbar">
    ...
    <div class="test-topbar-center">
      ...
      <div class="test-variant-sub">ВПР &middot; Математика &middot; 2026</div>
    </div>
...
@push('scripts')
<script>
  // Grade-specific названия тем ВПР, приходят из VprController::test().
  const TOPIC_NAMES = @json($topicNames ?? (object) []);
      ...
      async submitTest() {
        ...
        try {
          ...
          if (res.ok) {
            ...
            window.location.href = `/vpr/results/${this.attemptId}`;
          } else {
      ...
      confirmExit() {
        if (this.answeredCount > 0) {
          if (confirm('Выйти из теста? Ответы будут сохранены при завершении теста.')) {
            window.location.href = '{{ route("pwa.student.vpr.home") }}';
          }
        } else {
          window.location.href = '{{ route("pwa.student.vpr.home") }}';
        }
