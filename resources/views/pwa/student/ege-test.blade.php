...
<script>
  function testApp() {
    return {
      ...
      // Часть 2 профиля (задания 14–20 по плану КИМ 2027): ответ бывает
      // корнем с π, точным радикалом или множеством промежутков — числовым
      // полем его не ввести. Задание 13 (экономическая) отвечается числом
      // и осталось за границей: с 2027 года это первая часть.
      hasMathAnswer(task) {
        return Number(this.displayTaskNumber(task)) >= 14;
      },
      ...
      async submitTest() {
        ...
        try {
          ...
          if (res.ok) {
            ...
            window.location.href = `/ege-app/results/${this.attemptId}`;
          } else {
      ...
      confirmExit() {
        if (this.answeredCount > 0) {
          if (confirm('Выйти из теста? Ответы будут сохранены при завершении теста.')) {
            window.location.href = '{{ route("pwa.student.ege.home") }}';
          }
        } else {
          window.location.href = '{{ route("pwa.student.ege.home") }}';
        }
