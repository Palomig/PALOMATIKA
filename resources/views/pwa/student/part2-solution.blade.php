...
{{-- Длинные формулы масштабируются под ширину экрана, чтобы не было горизонтального скролла --}}
<script>
(function () {
  function fitFormulas() {
    var boxes = document.querySelectorAll('.sol-body .formula');
    for (var i = 0; i < boxes.length; i++) {
      var box = boxes[i];
      var disp = box.querySelector('.katex-display');
      var k = (disp && disp.querySelector('.katex')) || box.querySelector('.katex');
      if (!k) continue;

      // Сброс предыдущего масштабирования перед измерением.
      k.style.transform = '';
      k.style.transformOrigin = '';
      if (disp) disp.style.height = '';

      // Истинную ширину формулы измеряем в inline-block (иначе блок === ширине контейнера).
      var prevDisplay = k.style.display;
      k.style.display = 'inline-block';
      var contentWidth = k.getBoundingClientRect().width;
      k.style.display = prevDisplay;

      var avail = box.clientWidth - 4; // небольшой запас, чтобы не появлялся скроллбар
      if (contentWidth > avail && avail > 0) {
        var scale = avail / contentWidth;
        k.style.transformOrigin = 'center top';
        k.style.transform = 'scale(' + scale + ')';
        // Убираем пустоту, которая осталась бы под уменьшенной формулой.
        if (disp) disp.style.height = k.getBoundingClientRect().height + 'px';
      }
    }
  }
  ...
  function schedule() {
    fitFormulas();
    setTimeout(fitFormulas, 80);
    setTimeout(fitFormulas, 350);
  }
  ...
  window.addEventListener('resize', function () {
    ...
    resizeTimer = setTimeout(fitFormulas, 150);
  });
})();
</script>
@endsection
