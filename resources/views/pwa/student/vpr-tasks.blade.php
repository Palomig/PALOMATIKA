@extends('layouts.pwa')
@section('title', 'База заданий ВПР · ' . $grade . ' класс — palomatika')
...
<div class="page task-render-scope">
  ...
  <div style="display:flex;align-items:center;gap:12px;opacity:0;animation:fadeDown .3s ease forwards;">
    <a href="{{ route('pwa.student.vpr.home') }}" class="back-btn">‹</a>
    <div style="flex:1;">
      <div style="font-family:var(--display);font-size:16px;color:var(--text);">База заданий ВПР</div>
      <div style="font-size:11px;color:var(--muted);font-weight:700;margin-top:1px;">{{ $grade }} класс · {{ $maxTopic }} заданий</div>
    </div>
  ...
  <div class="topics-row">
    @foreach($topicIds as $tid)
      <a class="topic-pill {{ $selected === $tid ? 'active' : '' }}"
         href="{{ route('pwa.student.vpr.tasks', ['topic' => (int)$tid]) }}">
        {{ (int)$tid }}
      </a>
    @endforeach
  </div>
