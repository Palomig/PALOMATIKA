...
<html lang="ru">
<head>
    <title>База заданий · Алгебра — PALOMATIKA</title>
...
<div class="max-w-6xl mx-auto px-4 py-10">
    <header class="mb-8 flex items-start justify-between gap-4 flex-wrap">
        <div>
            <h1 class="exam-title text-3xl sm:text-4xl text-white">База заданий · Алгебра</h1>
            <p class="text-slate-400 mt-2">5–8 классы. Темы и задания добавляются по мере готовности.</p>
        </div>
        ...
        @if(empty($data['topics']))
            <div class="rounded-xl border border-dashed border-slate-800 bg-dark-light/20 p-6 text-center">
                ...
                <p class="text-slate-600 text-xs mt-1">
                    Добавь JSON-файлы в
                    <code class="bg-gray-800 px-2 py-0.5 rounded text-[11px]">storage/app/tasks/alg/grade_{{ $grade }}/topic_NN.json</code>
                </p>
            </div>
        @else
