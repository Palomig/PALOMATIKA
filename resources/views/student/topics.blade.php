...
<div x-data="topicsPage()">
    ...
    <div class="bg-dark-light rounded-2xl p-6 border border-gray-800 mb-6">
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
            <div>
                <div class="text-3xl font-bold text-coral" x-text="overallProgress + '%'"></div>
                <div class="text-gray-400 text-sm">общий прогресс</div>
            </div>
            <div>
                <div class="text-3xl font-bold text-green-400" x-text="completedTopics"></div>
                <div class="text-gray-400 text-sm">тем пройдено</div>
            </div>
            <div>
                <div class="text-3xl font-bold text-blue-400" x-text="totalTasks"></div>
                <div class="text-gray-400 text-sm">задач решено</div>
            </div>
            <div>
                <div class="text-3xl font-bold text-amber-400" x-text="avgAccuracy + '%'"></div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        <template x-for="topic in topics" :key="topic.id">
            <a :href="'/topics/' + topic.id" class="bg-dark-light rounded-xl p-6 border border-gray-800 hover:border-coral/50 transition cursor-pointer block">
                ...
                <h3 class="font-semibold text-white mb-2" x-text="topic.name"></h3>
                <p class="text-gray-400 text-sm mb-4" x-text="topic.description"></p>
                <div class="flex items-center justify-between text-sm mb-2">
                    <span class="text-gray-500">Прогресс</span>
                    <span class="font-medium text-gray-300" x-text="topic.progress + '%'"></span>
                </div>
                <div class="bg-dark rounded-full h-2">
                    <div class="rounded-full h-2 transition-all"
                         :class="topic.progress >= 100 ? 'bg-green-500' : 'bg-coral'"
    ...
    <div x-show="loading" class="text-center py-12">
        <svg class="animate-spin h-8 w-8 mx-auto text-coral" fill="none" viewBox="0 0 24 24">
            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"></path>
        </svg>
        <p class="text-gray-400 mt-2">Загрузка тем...</p>
    </div>
</div>
