...
@if($svgType === 'three_points')
    ...
    <div class="bg-slate-900/50 rounded-lg p-4">
        <svg viewBox="0 0 400 50" class="w-full max-w-md h-16 number-line">
            ...
            {{-- Линия --}}
            <line x1="20" y1="20" x2="380" y2="20" stroke="#3a5a7c" stroke-width="2.5" marker-end="url(#arrow_{{ $uniqueId }})"/>
            ...
            @foreach($positions as $i => $px)
                <circle cx="{{ $px }}" cy="20" r="4" fill="#7eb8da"/>
                <text x="{{ $px }}" y="42" text-anchor="middle" fill="#c8dce8" font-size="15" font-weight="bold" font-style="italic">{{ $labels[$i] ?? '' }}</text>
            @endforeach
        </svg>
    </div>
...
@elseif($svgType === 'four_points_abcd')
    ...
    <div class="bg-slate-900/50 rounded-lg p-4">
        <svg viewBox="0 0 400 55" class="w-full max-w-md h-16 number-line">
            ...
            {{-- Линия --}}
            <line x1="{{ $lineStart }}" y1="22" x2="{{ $lineEnd }}" y2="22" stroke="#3a5a7c" stroke-width="2.5" marker-end="url(#arrow_{{ $uniqueId }})"/>

            {{-- Деления и подписи чисел --}}
            @for($i = $minV; $i <= $maxV; $i++)
                @php $tx = $getX($i); @endphp
                <line x1="{{ $tx }}" y1="15" x2="{{ $tx }}" y2="29" stroke="#3a5a7c" stroke-width="2"/>
                <text x="{{ $tx }}" y="48" text-anchor="middle" fill="#c8dce8" font-size="14" font-weight="bold">{{ $i }}</text>
            @endfor

            {{-- Четыре точки A, B, C, D --}}
            @foreach($fourPts as $i => $ptVal)
                @php $px = $getX($ptVal); @endphp
                <circle cx="{{ $px }}" cy="22" r="4" fill="#7eb8da"/>
                <text x="{{ $px }}" y="10" text-anchor="middle" fill="#c8dce8" font-size="14" font-weight="bold" font-style="italic">{{ $labels[$i] ?? '' }}</text>
            @endforeach
        </svg>
    </div>
...
@elseif($svgType === 'two_points')
    ...
    <div class="bg-slate-900/50 rounded-lg p-4">
        <svg viewBox="0 0 370 55" class="w-full max-w-md h-16 number-line">
            ...
            <line x1="{{ $zeroX }}" y1="15" x2="{{ $zeroX }}" y2="29" stroke="#3a5a7c" stroke-width="2"/>
            <text x="{{ $zeroX }}" y="48" text-anchor="middle" fill="#d4a855" font-size="14" font-weight="bold">0</text>

            {{-- Две точки --}}
            @foreach($pts as $pt)
                @php $px = $getX($pt['value']); @endphp
                <circle cx="{{ $px }}" cy="22" r="4" fill="#7eb8da"/>
                <text x="{{ $px }}" y="10" text-anchor="middle" fill="#c8dce8" font-size="14" font-weight="bold" font-style="italic">{{ $pt['label'] }}</text>
            @endforeach
        </svg>
    </div>
...
@elseif($svgType === 'point_a_on_range')
    ...
    <div class="bg-slate-900/50 rounded-lg p-4">
        <svg viewBox="0 0 400 55" class="w-full max-w-md h-16 number-line">
            ...
            {{-- Линия --}}
            <line x1="{{ $lineStart }}" y1="20" x2="{{ $lineEnd }}" y2="20" stroke="#3a5a7c" stroke-width="2.5" marker-end="url(#arrow_{{ $uniqueId }})"/>
            ...
            @for($i = 0; $i <= $divisions; $i++)
                ...
                @endphp
                <line x1="{{ $tx }}" y1="13" x2="{{ $tx }}" y2="27" stroke="#3a5a7c" stroke-width="1.5"/>
                <text x="{{ $tx }}" y="45" text-anchor="middle" fill="#c8dce8" font-size="11">{{ $label }}</text>
            @endfor

            {{-- Точка A --}}
            @php $px = $getX($pointVal); @endphp
            <circle cx="{{ $px }}" cy="20" r="4" fill="#7eb8da"/>
            <text x="{{ $px }}" y="8" text-anchor="middle" fill="#c8dce8" font-size="14" font-weight="bold" font-style="italic">{{ $pointLabel }}</text>
        </svg>
    </div>
    ...
    <div class="bg-slate-900/50 rounded-lg p-4">
        <svg viewBox="0 0 320 55" class="w-full max-w-md h-16 number-line">
            ...
            @for($i = $minTick; $i <= $maxTick; $i++)
                @php $tx = $getX($i); @endphp
                <line x1="{{ $tx }}" y1="15" x2="{{ $tx }}" y2="29" stroke="#3a5a7c" stroke-width="1.5"/>
            @endfor

            {{-- Подписи только 0 и 1 --}}
            <text x="{{ $getX(0) }}" y="48" text-anchor="middle" fill="#d4a855" font-size="13" font-weight="bold">0</text>
            <text x="{{ $getX(1) }}" y="48" text-anchor="middle" fill="#c8dce8" font-size="13" font-weight="bold">1</text>

            {{-- Точка --}}
            @php $px = $getX($pointVal); @endphp
            <circle cx="{{ $px }}" cy="22" r="4" fill="#7eb8da"/>
            <text x="{{ $px }}" y="10" text-anchor="middle" fill="#c8dce8" font-size="14" font-weight="bold" font-style="italic">{{ $pointLabel }}</text>
        </svg>
    </div>
@endif
