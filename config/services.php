...
return [
    ...
    */
    ...
    /*
    |--------------------------------------------------------------------------
    | hw-photos — внешнее хранилище фото решений домашки
    |--------------------------------------------------------------------------
    |
    | Сервис на VPS (см. модуль homework). Ученик грузит снимок напрямую туда,
    | Laravel только подписывает токены. Без `secret` фича молча выключена и
    | работает прежний путь — файл сохраняется на хостинге.
    |
    */

    'hw_photos' => [
        'url'        => env('HW_PHOTOS_URL', 'https://palomig.ru/hw-photos'),
        'secret'     => env('HW_PHOTOS_SECRET'),
        'upload_ttl' => (int) env('HW_PHOTOS_UPLOAD_TTL', 900),
        'read_ttl'   => (int) env('HW_PHOTOS_READ_TTL', 3600),
    ],
