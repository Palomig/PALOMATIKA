...
class BakeSvgToJson extends Command
{
    ...
    public function handle(): int
    {
        ...
        // Topic 07 (number lines) - reads from main JSON
        if ($topicId === '07') {
            return $this->handleTopic07($dryRun);
        }
    ...
    /**
     * Handle topic 07 (number lines) - generates SVG for coordinate line tasks
     */
    private function handleTopic07(bool $dryRun): int
    {
        $inputPath = storage_path("app/tasks/topic_07.json");
        ...
        $this->info("Reading topic 07 data from: topic_07.json");
        ...
        $renderer = new NumberLineSvgRenderer();
        $count = 0;
        $errors = 0;
        $skipped = 0;
        ...
        foreach ($data['blocks'] as $blockIndex => &$block) {
            ...
            foreach ($block['zadaniya'] as $zadanieIndex => &$zadanie) {
                ...
                // For simple_choice with svg_type (three_points), render once per zadanie
                if (($zadanie['type'] === 'simple_choice') && isset($zadanie['points'])) {
                    try {
                        $svg = $renderer->render($svgType, [], $zadanie);
                        $zadanie['svg'] = $svg;
                        $this->line("    ✓ Zadanie-level SVG: {$svgType}");
                        $count++;
                    } catch (\Exception $e) {
                ...
                foreach ($zadanie['tasks'] as $taskIndex => &$task) {
                    try {
                        $svg = $renderer->render($svgType, $task, $zadanie);

                        if (empty($svg)) {
                            $this->warn("    ⚠️  Task {$task['id']}: Empty SVG generated");
                            $skipped++;
                            continue;
                        }
