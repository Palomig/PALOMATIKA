...
class EgeStudentController extends Controller
{
    ...
    public function taskDatabase(Request $request)
    {
        ...
        // Уровень: профиль (1–20 по плану КИМ 2027, две части) или база
        // (1–21, все с кратким ответом). У базы делить нечего —
        // развёрнутых заданий в ней нет.
        $level = $this->resolveLevel($request);
        $isBase = $level === EgeTaskDataService::LEVEL_BASE;
        $taskData = new EgeTaskDataService($level);

        // Часть экзамена: 1–13 дают краткий ответ, 14–20 — развёрнутый.
        // Деление приходит от самого ФИПИ (qkind) и совпадает с номерами.
        // С плана 2027 граница сдвинулась: экономическая задача стала
        // тринадцатой и отвечается числом, то есть это первая часть.
        ...
        if ($isBase) {
            ...
            $partHint = $part === 2 ? 'Задания 14–20 · развёрнутый ответ' : 'Задания 1–13 · краткий ответ';
        }
