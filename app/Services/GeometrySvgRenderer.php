...
SVG;
    ...
    private function renderTrapezoidCircumscribedAroundCircle(): string
    {
        ...
        // Произвольная (НЕ равнобедренная) трапеция
        // Верхнее и нижнее основания - горизонтальные касательные к окружности
        // Боковые стороны - наклонные касательные с РАЗНЫМ наклоном
        // Вычислено: левая сторона круче, правая положе
    ...
    private function renderQuadCircumscribedAroundCircle(): string
    {
        ...
        // Все 4 стороны находятся на расстоянии r от центра O
        //
        // Нижняя сторона AD: горизонталь y = O[y] + r = 145
        // Верхняя сторона BC: горизонталь y = O[y] - r = 55
        // Боковые стороны AB и CD: наклонные касательные
        //
        // Для левой стороны AB (от A(ax, 145) до B(bx, 55)):
        // Расстояние от O до прямой = |9900 - 45(ax + bx)| / sqrt(8100 + (bx-ax)²) = 45
        // При ax = 50: bx ≈ 76
        //
        // Для правой стороны CD (от C(cx, 55) до D(dx, 145)):
        // При dx = 180: cx ≈ 139
