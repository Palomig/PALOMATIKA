...
/**
 * Сервис данных банка заданий по геометрии (7–9 классы).
 * Данные: storage/app/tasks/geom/grade_{N}/topic_NN.json
 *
 * Темы определяются динамически по существующим topic_NN.json
 * (см. AlgTaskDataService — та же модель).
 */
class GeomTaskDataService
{
    ...
    public function __construct(protected int $grade)
    {
        if (!in_array($grade, self::GRADES, true)) {
            throw new \InvalidArgumentException("Класс {$grade} не поддерживается для геометрии");
        }

        $this->basePath = storage_path("app/tasks/geom/grade_{$grade}");
    ...
    {
        $cacheKey = "geom_g{$this->grade}_topic_{$topicId}";
