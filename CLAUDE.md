...
- Изменения **не попадут на production, пока они не закоммичены и не запушены на GitHub**. Локальные правки сами по себе деплой не запускают.
- После завершения задачи обязательно сделать `git push` в текущую `claude/*` ветку и проверить, что автомёрдж дошёл до `main`
