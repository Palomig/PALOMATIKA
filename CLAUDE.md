...
- Ветки `claude/*` **автоматически** мержатся в `main` — и на этом GitHub Actions заканчивается
- **Доставка на прод — ручная, сразу после пуша:** `scripts/deploy-prod.sh` (заливка по FTP со сверкой MD5 + `deploy:refresh`). Пуш сам по себе на прод ничего не выкладывает
