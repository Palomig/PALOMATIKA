...
**`main` — это ровно то, что на проде. Ничего не попадает в `main` само.**

- Ветки `claude/*` авто-мержатся в **`develop`** — интеграционную песочницу. На прод оттуда не едет ничего
- Фичу ответвляй от `main`, не от `develop`: `git worktree add ../palomatika-<фича> -b claude/<фича> origin/main`
- Когда Стас сказал «готово» — `scripts/promote.sh claude/<фича>`: мёрж ветки в `main` + доставка на прод одной командой
- `develop` в `main` не мержится **никогда** — иначе на прод уедет вся незрелая работа разом
- Доставка вручную: `scripts/deploy-prod.sh` (FTP + сверка MD5 + `deploy:refresh`). Пуш сам по себе на прод ничего не выкладывает
- Скрипт заливает файлы **из рабочего дерева** — оно обязано стоять на деплоимой ревизии. Проверка встроена в скрипт
