...
// Lesson invite join (public — гость редиректится на login, после joinByToken идёт в student PWA)
Route::get('/lesson/join/{token}', [\App\Http\Controllers\LessonJoinController::class, 'join'])
    ->name('lesson.join')
...
Route::prefix('api/oge')->middleware(['auth', 'role:student,admin'])->group(function () {
    ...
    Route::post('/attempts/{attempt}/telemetry', [OgeAttemptController::class, 'telemetry'])->name('api.oge.attempt.telemetry');
